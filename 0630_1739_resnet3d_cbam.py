...
def train(model, optimizer, train_loader, val_loader, scheduler, args, device="cuda:0"):
    ...
    for epoch in range(1, args['epoch'] + 1):
        ...
        for videos, labels in tqdm(iter(train_loader)):
            ...
            running_corrects += torch.sum(train_pred == labels.squeeze())
        ...
        with torch.no_grad():
            ...
            for videos, labels in tqdm(iter(val_loader)):
                ...
                running_corrects += torch.sum(valid_pred == labels.squeeze())
        ...
        print(f"Validation loss: {curr_valid_loss:.5f} Validation accuracy: {epoch_acc:.5f} F1Score: {val_score:.5f}  acc: {acc:.5f}")
