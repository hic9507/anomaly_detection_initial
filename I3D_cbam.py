...
args = {"data_folder": "D:\\abnormal_detection_dataset\\UBI_FIGHTS\\videos\\videos\\",
        ...
        "decay_rate": 0.998, "num_workers": 4, "img_size": (224, 224), "img_depth": 3, "FPS": 110} # decay_rate 원래 0.98 "img_size": (320, 240)
...
#         self.fc = nn.Linear(sum(self.inception5b.out_channels), num_classes)
#
#     def forward(self, x):
#         x = self.conv1(x)
#         x = self.inception1(x)
...
# model = I3D_with_CBAM(num_classes=args["num_class"]).to(device)
# summary(model, (3, 1, 360, 240))  # 입력 형태에 맞게 수정해주세요
gc.collect()
...
class EarlyStopping:
    ...
    def __init__(self, patience=7, verbose=False, delta=0, path=f"{model_save_folder}I3D_cbam_UBI-FIGHTS-VIDEO_224x224_model_early.pth"):
        """
        Args:
...
def train(args, device:"cuda:0"):
    ...
    train_generator = DataLoader(train_dataset, batch_size=args["batch_size"], shuffle=False,
                                 num_workers=args["num_workers"], pin_memory=True)  # , sampler=sampler , collate_fn=custom_collate
    ...
    for epoch in range(args["epoch"]):
        ...
        for i, train_data in tqdm(enumerate(train_generator)):
            ...
            train_loss = criterion(input=outputs, target=labels)
        ...
        if best_accuracy < curr_test_accuracy:
            torch.save(model.state_dict(), f"{model_save_folder}I3D_cbam_UBI-FIGHTS_224x224_videocapture_model.pth")
    ...
    plt.xlim([1, int(cnt)])
    plt.legend(['train', 'valid'], loc='upper left')
    plt.savefig(args["graphs_folder"] + f"UBI-FIGHTS_video_224x224_acc_graph_after_train.png")
    plt.show()
    ...
    plt.xlim([1, int(cnt)])
    plt.legend(['train', 'valid'], loc='upper left')
    plt.savefig(args["graphs_folder"] + f"UBI-FIGHTS_video_224x224_loss_graph_after_train.png")
    plt.show()
    ...
    plt.savefig(args["graphs_folder"] + f"UBI-FIGHTS_video_224x224_AUC_graph.png")
    # plt.show()
    ...
    plt.legend(loc='lower right')
    plt.savefig(args["graphs_folder"] + f"UBI-FIGHTS_video_224x224_AUC_graph_after_train.png")
    plt.show()
