...
args = {"data_folder": "D:\\abnormal_detection_dataset\\UBI_FIGHTS\\videos\\videos\\",
        "graphs_folder": "./graph_resnet3d/UBI-FIGHTS-VIDEO/", "epoch": 2, "batch_size": 1, "num_class": 1, "learning_rate": 0.001,  # 원래 1e-4
        "decay_rate": 0.0998, "num_workers": 4, "img_size": (640, 360), "img_depth": 3, "FPS": 50, "SEED": 41} # decay_rate 원래 0.98 "img_size": (320, 240)
...
def train(model, optimizer, train_loader, val_loader, scheduler, device):
    ...
    best_model = None
    best_epoch = 0
    train_acc, train_loss, val_acc, val_losses = [], [], [], []
    y_pred, y_true = [], []
    for epoch in range(1, args['epoch'] + 1):
        ...
        val_loss, val_score, acc, _val_loss = validation(model, criterion, val_loader, device)
        ...
        _train_loss = sum(train_loss) / len(train_loader)
        train_loss.append(_train_loss)

        curr_train_loss = sum(train_loss) / (int(epoch) + 1)
        curr_valid_loss = sum(val_loss) / (int(epoch) + 1)

        # train_acc = np.mean([val for sublist in train_acc for val in sublist]) if train_acc else 0
        # val_loss = np.mean([val for sublist in val_loss for val in sublist]) if val_loss else 0

        print(f"Epoch {epoch}/{args['epoch']}")
        # print(type(_train_loss), type(train_acc), type(val_loss), type(val_score), type(acc)) # <class 'numpy.float64'> <class 'list'> <class 'list'> <class 'numpy.float64'> <class 'numpy.float64'>
        print(f'Train Loss : {_train_loss:.5f}  Train Acc : {epoch_acc}  Val Loss : {_val_loss}  Val Acc :{acc:.5f}  Val F1 : {val_score:.5f}')
        # print(f'Train Loss : {curr_train_loss:.5f}  Train Acc : {epoch_acc}  Val Loss : {curr_valid_loss}  Val Acc :{acc:.5f}  Val F1 : {val_score:.5f}')
...
def validation(model, criterion, val_loader, device):
    ...
    with torch.no_grad():
        for videos, labels in tqdm(iter(val_loader)):
            ...
            preds_ = torch.argmax(logit, dim=1)
            running_corrects += torch.sum(preds_ == labels.squeeze())

            val_epoch_acc_ = running_corrects / (len(val_loader) * args["batch_size"])
            val_acc.append(val_epoch_acc_)
    ...
    return val_loss, val_score, acc, _val_loss
