...
train_dataset = CustomDataset(train_x, train_y, transform=transforms.Compose([transforms.ToTensor(), transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5)),
                                                                 transforms.RandomVerticalFlip(), transforms.RandomHorizontalFlip(), transforms.RandomRotation(180, expand=False)]))
train_loader = DataLoader(train_dataset, batch_size=args['batch_size'], shuffle=True, num_workers=args["num_workers"])

val_dataset = CustomDataset(val_x, val_y, transform=transforms.Compose([transforms.ToTensor(), transforms.Normalize((0.5, 0.5, 0.5), (0.5, 0.5, 0.5)),
                                                                 transforms.RandomVerticalFlip(), transforms.RandomHorizontalFlip(), transforms.RandomRotation(180, expand=False)]))
...
def train(model, device, args, train_loader, val_loader):
    ...
    for epoch in range(1, args['epoch'] + 1):
        ...
        print(f"train_loss_avg :  {train_loss_avg:.5f} train_acc_avg:  {train_acc_avg:.5f}")
        ...
        print(f'test_loss.item() : {test_loss.item():.5f}  val_epoch_acc_.item() : {val_epoch_acc_.item():.5f} acc: {acc:.5f}')  # 최적
        ...
        print(f'val_loss_avg : {val_loss_avg:.5f}  val_acc_avg : {val_acc_avg:.5f} accuracy: {accuracy:.5f} val_epoch_acc_s: {val_epoch_acc_s:.5f}')  # 최적
        ...
        test_acc_for_graph.append(val_loss_avg_gpt)
