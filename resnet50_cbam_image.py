...
args = {"data_folder": "D:\\abnormal_detection_dataset\\UBI_FIGHTS\\videos\\videos\\",
        "graphs_folder": "./graph_I3D/UBI-FIGHTS-VIDEO/", "epoch": 50, "batch_size": 16, "num_class": 1, "learning_rate": 0.001,  # 원래 1e-4
    ...
    def read_folder(self):
        ...
        for x in glob.glob(folder_path + "**", recursive=True):  # 해당 폴더의 하위 폴더까지 탐색
            if not x.endswith('mp4'): # rwf2000: mp4
                continue
            class_idx = self.classes.index(x.split('\\')[-2])  # 클래스 이름(Violence, Nonviolence)의 인덱스 저장
...
def train(args, device="cuda:0"):
    ...
    for epoch in range(args["epoch"]):
        ...
        for i, train_data in tqdm(enumerate(train_generator)):
            ...
            running_corrects += torch.sum(pred == y_train.squeeze())

        train_acc = running_corrects.item() / (len(train_generator) * args["batch_size"])

        train_loss_list.append(train_loss)
        train_acc_list.append(train_acc)

        print(f"Epoch {epoch + 1}/{args['epoch']}")
        print(f"Train_loss :  {train_loss:.5f}, 'Train accuracy: , {train_acc:.5f}")
        ...
        with torch.no_grad():
            for i, test_data in tqdm(enumerate(test_generator)):
                ...
                test_loss = criterion(input=output.to(torch.float32), target=y_test_tensor)
                ...
                pred = torch.sigmoid(output) > 0.5
                running_corrects += torch.sum(pred == y_test.squeeze())
        ...
        validation_accuracy = running_corrects.item() / (len(test_generator) * args["batch_size"])

        test_loss_list.append(test_loss)
        test_acc_list.append(validation_accuracy)

        print(f"validation loss: {test_loss:.4f} \t validation accuracy: {validation_accuracy:.4f}")
    ...
    plt.savefig(args["graphs_folder"]+f"auc.png")
    plt.show()
    ...
    plt.savefig(args["graphs_folder"]+f"/acc.png")
    plt.show()
    ...
    plt.xlim([1, args["epoch"]])
    plt.legend(['train', 'valid'], loc='upper left')
    plt.savefig(args["graphs_folder"]+f"/loss.png")
    plt.show()
