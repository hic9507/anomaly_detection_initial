...
#         "graphs_folder": "./graph/RWF2000/", "epoch": 50, "batch_size": 8, "num_class": 1, "learning_rate": 1e-4,
#         "decay_rate": 0.98, "num_workers": 4, "img_size": (640, 360), "img_depth": 3, "FPS": 30}

args = {"data_folder": "D:\\abnormal_detection_dataset\\UBI_FIGHTS\\videos\\videos\\",
        "graphs_folder": "./graph_I3D/UBI-FIGHTS-VIDEO/", "epoch": 50, "batch_size": 16, "num_class": 2, "learning_rate": 0.001,  # 원래 1e-4
        "decay_rate": 0.0001, "num_workers": 4, "img_size": (640, 360), "img_depth": 3, "FPS": 600} # decay_rate 원래 0.98 "img_size": (320, 240)
...
class ResNet50(nn.Module):
    ...
    def __init__(self, image_depth=3, num_classes=args["num_class"]):
        ...
        self.lstm = nn.LSTM(input_size=4096, hidden_size=256, batch_first=True)
        # self.linear = nn.Linear(512*self.expansion*2, num_classes) # img_size: (640, 360) 일 때 기존 코드임!
        self.linear = nn.Linear(256, num_classes) # img_size: (640, 360) 일 때
...
###### CustomDataset 정의
    def forward(self, x):
        ...
        # Flatten starting from the first dimension (keeping batch dimension)
        x = nn.Flatten(start_dim=1)(x)
...
class EarlyStopping:
    ...
    def __init__(self, patience=7, verbose=False, delta=0, path=f"{model_save_folder}UBI-FIGHTS_640x360_videocapture_model_earlywithout_Cbam.pth"):
        """
        Args:
...
def get_accuracy(y_true, y_prob):
    y_prob = torch.sigmoid(torch.tensor(y_prob)).numpy()
...
def train(args, device="cuda:0"):
    ...
    train_generator = DataLoader(train_dataset, batch_size=args["batch_size"], shuffle=False, num_workers=args["num_workers"], pin_memory=True, drop_last=True)  # , sampler=sampler
    test_generator = DataLoader(test_dataset, batch_size=args["batch_size"], shuffle=False, num_workers=args["num_workers"], pin_memory=True, drop_last=True)
    ...
    for epoch in range(args["epoch"]):
        ...
        for i, train_data in tqdm(enumerate(train_generator)):
            ...
            y_train_numpy = np.array(y_train.detach().cpu().numpy())
            y_train_tensor = torch.from_numpy(y_train_numpy).float().to(device)
            ...
            # acc = get_accuracy(y_true=y_train.detach().cpu().numpy(), y_prob=output.detach().cpu().numpy())
        ...
        epoch_acc = running_corrects.item() / (len(train_generator) * args["batch_size"])
        ...
        # print(f"Train_loss :  {curr_train_loss:.5f} Train accuracy:  {acc:.5f}")
        ...
        with torch.no_grad():
            for i, test_data in tqdm(enumerate(test_generator)):
                ...
                # y_test_tensor = torch.from_numpy(y_test_numpy).float().to(device)
                # test_loss = criterion(input=output.to(torch.float32), target=y_test_tensor) # bceloss
                ...
                # acc = get_accuracy(y_true=y_test.detach().cpu().numpy(), y_prob=output.detach().cpu().numpy())
            ...
            epoch_acc = running_corrects.item() / (len(test_generator) * args["batch_size"])
        ...
        # print(f"Validation loss: {curr_test_loss:.5f} Validation accuracy: {acc:.5f}")
        ...
        if best_accuracy < epoch_acc:
            torch.save(model.state_dict(), f"{model_save_folder}UBI-FIGHTS_640x360_videocapture_modelwithout_Cbam.pth")
            best_accuracy = epoch_acc
            print('Current model has best valid accuracy')


        valid_loss = np.average(test_loss_list)
        valid_losses = []
    ...
    # y_pred = np.array(y_pred, dtype=np.int64) #.sum(axis=0)           # 원래 코드
    # y_true = np.array(y_true, dtype=np.int64) #.max(axis=1)           # 원래 코드

    y_pred = np.array(y_pred, dtype=object) #.sum(axis=0)
    y_true = np.array(y_true, dtype=object) #.max(axis=1)

    print('Before reshape')
    print(type(y_pred), type(y_true))
    print(y_pred.shape, y_true.shape) # (2, 16, 1) (6, 16, 1)
    ...
    fpr, tpr, _ = roc_curve(np.array(y_true), np.array(y_pred))
    ...
    plt.savefig(args["graphs_folder"]+f"AUC_graph0517without_Cbam.png")
    plt.show()
    ...
    plt.savefig(args["graphs_folder"]+f"/rwf2000_acc_graph30900517without_Cbam.png")
    plt.show()
    ...
    plt.xlim([1, args["epoch"]])
    plt.legend(['train', 'valid'], loc='upper left')
    plt.savefig(args["graphs_folder"]+f"/rwf2000_loss_graph30900517without_Cbam.png")
    plt.show()
