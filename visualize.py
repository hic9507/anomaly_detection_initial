...
# path = 'C:/Users/user/Desktop/env/ResNet-50-CBAM-PyTorch/test/rwf2000/'
# path = './test_I3D_cbam/UBI_FIGHTS/'
path = './test_I3D_cbam/UBI_FIGHTS - copy/'

# args = {"data_folder": "D:\\abnormal_detection_dataset\\UBI_FIGHTS\\videos\\videos\\",
#         "graphs_folder": "./trained_model/RWF2000/", "epoch": 50, "batch_size": 32, "num_class": 2, "learning_rate": 1e-4,
...
class LoadInputImages(Dataset):
    ...
    def __init__(self, input_folder=args["data_folder"], image_size=args["img_size"], image_depth=3, train=False, transform=None):
    ...
    def __getitem__(self, idx):
        ...
        frames = torch.FloatTensor(np.array(frames)).permute(3, 0, 1, 2)  # (T, H, W, C) -> (T, C, H, W)
...
def viz(args, device="cuda:0"):
    # model_save_folder = './trained_model/'
    ...
    try:
        ...
        new_state_dict = OrderedDict() # 가중치에서 불필요한 부분 제거 후 필요한 것만 저장
    ...
    for i, image in tqdm(enumerate(data_generator)):
        ...
        output = model(image)  # model(image)의 결과가 cnn 필터와 model output 두 개임
        cnn_filters = output
        ...
        heatmap = np.asarray(cv2.applyColorMap(cv2.normalize(cnn_combined_filter, None, alpha=0, beta=255, norm_type=cv2.NORM_MINMAX, dtype=cv2.CV_8U),
                            cv2.COLORMAP_JET), dtype=np.float32) # 위에서 병합한 CNN 필터를 적용한 히트맵을 생성한다. cv2.normalize() 함수를 이용해 cnn_combined_filter 값의 범위를 0~255 사이로 정규화하고, cv2.COLORMAP_JET 컬러 맵을 적용하여 컬러 이미지로 변환한다cv2.applyColorMap() 함수를 이용해 히트맵을 생성하는데 이를 위해 컬러맵 적용.
        ...
        # input_img = cv2.resize(image.squeeze(0).permute(1, 2, 0).cpu().numpy(), (args["img_size"])) # squeeze(0)을 이용해 차원을 축소하고, permute() 함수를 이용해 차원 순서를 변경해 입력 이미지 리사이즈
        # input_img = cv2.resize((image.squeeze(0).permute(0, 2, 3, 1).detach().cpu().numpy().astype(np.uint8)), dsize=(224,224)) # squeeze(0)을 이용해 차원을 축소하고, permute() 함수를 이용해 차원 순서를 변경해 입력 이미지 리사이즈
        input_img = cv2.resize((image.squeeze(0).detach().cpu().numpy().astype(np.uint8)), dsize=(224,224)) # squeeze(0)을 이용해 차원을 축소하고, permute() 함수를 이용해 차원 순서를 변경해 입력 이미지 리사이즈
        # input_img = cv2.resize((image.permute(0, 1, 2, 3).detach().cpu().numpy().astype('float32')), dsize=(224,224)) # squeeze(0)을 이용해 차원을 축소하고, permute() 함수를 이용해 차원 순서를 변경해 입력 이미지 리사이즈
        # input_img = cv2.resize((image.unsqueeze(-1).detach().cpu().numpy().astype('float32')), dsize=(224,224)) # squeeze(0)을 이용해 차원을 축소하고, permute() 함수를 이용해 차원 순서를 변경해 입력 이미지 리사이즈

        # 원본 이미지에 필터(히트맵)를 오버레이(씌워서)하여 히트맵 생성
